...
*/
	...
	public function recuperer(){
		$req = "SELECT visiteur.id, lignefraisforfait.idFraisForfait, fichefrais.montantValide, lignefraisforfait.mois
	...
	/** 4.a) recupère les frais pour une annee */
	public function fraisannee($moisAnnee)
	{
		$req = "SELECT lignefraisforfait.idVisiteur, lignefraisforfait.idFraisForfait 
				FROM lignefraisforfait 
				WHERE lignefraisforfait.mois = '$moisAnnee'";
		$stmt = $this->monPdo->prepare($req);
		$stmt->execute();	
		$resultats = $stmt->fetchAll(PDO::FETCH_OBJ);
		return $resultats;
	}
	/** 4.b) recupère les frais pour un visiteur */
	public function fraisvisiteur($visiteur){
		$req = "SELECT DISTINCT visiteur.nom, visiteur.prenom, lignefraisforfait.idVisiteur, lignefraisforfait.mois, lignefraisforfait.idFraisForfait
		FROM visiteur
		INNER JOIN lignefraisforfait ON visiteur.id = lignefraisforfait.idVisiteur
		WHERE visiteur.nom='$visiteur'";
	$stmt = $this->monPdo->prepare($req);
	$stmt->execute();
	$resultats = $stmt->fetchAll(PDO::FETCH_OBJ);
	return $resultats;
	...
	/** 4.c) recupère les frais pour un type */
	public function fraistype($type)
	{
		$req = "SELECT visiteur.id AS visiteur, fichefrais.montantValide AS montant, lignefraisforfait.mois 
				FROM lignefraisforfait 
				INNER JOIN fichefrais ON lignefraisforfait.idVisiteur = fichefrais.idVisiteur 
				INNER JOIN visiteur ON fichefrais.idVisiteur = visiteur.id 
				WHERE lignefraisforfait.idFraisForfait = '$type'";
		$stmt = $this->monPdo->prepare($req);
		$stmt->execute();
		$resultats = $stmt->fetchAll(PDO::FETCH_OBJ);
		return $resultats;
